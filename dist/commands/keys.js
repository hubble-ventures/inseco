...
import { buildSnapshot, diffKeyNames, emittedNamesFor, hasKeyChange, KEYS_SCHEMA_VERSION, serializeSnapshot, } from "../keys.js";
...
export async function runKeys(options) {
    ...
    console.error(`# ${manifest.id}${profileNote}: declared emitted keys (static; no vault access)`);
...
function checkSnapshot(repoRoot, current) {
    const path = join(repoRoot, SNAPSHOT_FILENAME);
    if (!existsSync(path)) {
        throw new Error(`No ${SNAPSHOT_FILENAME} to check against. Create it:\n` +
    ...
    if (committed.schemaVersion !== current.schemaVersion) {
        throw new Error(`${SNAPSHOT_FILENAME} schemaVersion ${committed.schemaVersion} != ` +
    ...
    for (const line of drift)
        console.error(`   ${line}`);
    throw new Error(`emitted key set drifted from ${SNAPSHOT_FILENAME}. ` +
...
function diffSnapshots(committed, current) {
    const lines = [];
    const ids = [
        ...new Set([
    ...
    for (const id of ids) {
        ...
        for (const profile of profiles) {
            ...
            if (!hasKeyChange(diff))
                continue;
            for (const k of diff.added)
                lines.push(`+ ${id} ${profile}: ${k}`);
...
function printSnapshotText(snapshot) {
