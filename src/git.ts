 ...
 */
export function loadManifestAtRef(
  cwd: string,
  packageDir: string,
  ref: string
  ...
  const found: { filename: string; raw: string }[] = [];
  for (const filename of MANIFEST_FILENAMES) {
    const repoRelPath = repoRelDir ? `${repoRelDir}/${filename}` : filename;
    const raw = showFileAtRef(cwd, ref, repoRelPath);
    if (raw !== null) found.push({ filename, raw });
  }
  if (found.length === 0) return null;
  if (found.length > 1) {
    throw new Error(
      `Ambiguous secrets manifest for '${repoRelDir}' at ${ref}: found ` +
