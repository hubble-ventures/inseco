import { resolveCompiledFolders, type SecretsManifest } from "./manifest.js";
 ...
 */
export function emittedKeyNames(folders: CompiledFolder[]): string[] {
  const names = new Set<string>();
  for (const folder of folders) {
    for (const key of folder.keys) {
      names.add(key.key);
      for (const alias of key.aliases) names.add(alias);
    }
  }
  return [...names].sort();
}
 ...
 */
export function emittedNamesFor(
  manifest: SecretsManifest | null,
  profile?: string
): string[] {
  if (!manifest) return [];
  if (profile !== undefined && !manifest.profiles?.[profile]) return [];
  return emittedKeyNames(resolveCompiledFolders(manifest, profile));
}

export type KeySetDiff = { added: string[]; removed: string[] };

/** Names in `to` but not `from` (added) and in `from` but not `to` (removed). */
export function diffKeyNames(from: string[], to: string[]): KeySetDiff {
...
export function hasKeyChange(diff: KeySetDiff): boolean {
  return diff.added.length > 0 || diff.removed.length > 0;
}

/** Every profile label to compare for a package: the default plus named profiles, sorted. */
export function profileLabels(manifest: SecretsManifest): string[] {
  return [
...
export function profileArg(label: string): string | undefined {
  return label === DEFAULT_PROFILE_LABEL ? undefined : label;
}

// A committable, value-free snapshot of every emitted key name, per package and
// profile. Static and environment-independent by construction (see ADR 0001).
export type KeysSnapshot = {
  schemaVersion: number;
  mode: "static";
  // packageId -> profileLabel -> sorted emitted names.
  packages: Record<string, Record<string, string[]>>;
};

/** Build the static emitted-names snapshot for a set of loaded manifests. */
export function buildSnapshot(
  manifests: { id: string; config: SecretsManifest }[]
): KeysSnapshot {
  const packages: Record<string, Record<string, string[]>> = {};
  for (const { id, config } of [...manifests].sort((a, b) =>
 ...
 */
export function serializeSnapshot(snapshot: KeysSnapshot): string {
