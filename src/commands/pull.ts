...
export async function runPull(options: PullOptions): Promise<void> {
  ...
  // tool they don't use.
  const needsInfisical = manifests.some(
    (m) => !shouldSkipInfisicalPull(m.config, options.force)
  );

  if (needsInfisical && !commandExists("infisical")) {
    throw new Error(
      "infisical CLI not found — install and run: infisical login"
    );
  }
