...
describe("aliases", () => {
  ...
  it("emits every target across repeated single-target alias entries", () => {
    const folders = compileTree([
      {
        clerk: [
  ...
  it("collects aliases across folders (with provenance)", () => {
    const folders = compileTree([
      ...
      {
        posthog: [
          { POSTHOG_PROJECT_TOKEN: "VITE_PH" },
          { POSTHOG_PROJECT_TOKEN: "NEXT_PH" },
        ],
      },
    ]);
    expect(resolveAliases(folders)).toEqual([
      { source: "CLERK_PUBLISHABLE_KEY", targets: ["VITE_CLERK"] },
      { source: "POSTHOG_PROJECT_TOKEN", targets: ["VITE_PH"] },
      { source: "POSTHOG_PROJECT_TOKEN", targets: ["NEXT_PH"] },
    ]);
  });
});
